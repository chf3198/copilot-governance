...
const fs = require('fs');
// #1948 P1.2: objective test-floor reconcile, surfaced ADVISORY (never blocks preflight).
const { advisoryLines: testFloorAdvisory } = require('./test-floor-classifier');
...
// #1948 P1.2: the raw changed-file set (not just sibling specs) for floor derivation.
// Prefers an explicit --files=/--diff-files= list; otherwise the merge-base..HEAD diff.
function resolveChangedFiles(argv, cwd = ROOT) {
  const explicit = (argv.find(a => a.startsWith('--files=') || a.startsWith('--diff-files=')) || '')
    .replace(/^--(files|diff-files)=/, '').split(',').filter(Boolean);
  if (explicit.length) return explicit;
  const base = spawnSync('git', ['merge-base', 'HEAD', 'main'], { cwd, encoding: 'utf8' });
  if (base.status !== 0) return [];
  const diff = spawnSync('git', ['diff', '--name-only', '--diff-filter=ACMR',
    base.stdout.trim(), 'HEAD'], { cwd, encoding: 'utf8' });
...
async function run(argv = process.argv.slice(2), opts = {}) {
  ...
  // #1948 P1.2: objective test-floor reconcile — ADVISORY, never blocks the preflight.
  // Promotion to a blocking gate is replay-eval-gated (#1771/#1875) and out of scope.
  const declared = (argv.find(a => a.startsWith('--declared=')) || '').replace('--declared=', '');
  const changedForFloor = opts.changedFiles || resolveChangedFiles(argv, ROOT);
  const floor = (opts.testFloorAdvisory || testFloorAdvisory)(declared, changedForFloor, { ticket });
  console.log('\n=== test-floor (advisory, non-blocking) ===');
...
module.exports = {
  ...
  deriveTestPaths, resolveTestScope, resolveChangedFiles, interpretReview, runFleetReview,
  partitionSpecs,
};
