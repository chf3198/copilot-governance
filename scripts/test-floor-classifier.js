 ...
 *  Canonical flag = TEST_FLOOR_DISABLED (per #1948 cross-family consensus 826a5fd62c0d866d);
 *  the older Epic-prose name MEGINGJORD_TEST_FLOOR_ADVISORY is a documented legacy alias. */
function isDisabled(env = process.env) { return env.TEST_FLOOR_DISABLED === '1'; }

// #1948 P1.2 gate wiring: advisory (NON-BLOCKING) presentation of a reconcile() for a
// changed-file set, for a preflight/handoff to print. Honors the TEST_FLOOR_DISABLED
// rollback flag. Pure aside from the optional content read reconcile() already performs
// (omit opts.root to keep it fully path-deterministic). Never throws, never blocks — the
// promotion to a blocking gate is replay-eval-gated and out of scope here.
/**
 * @param {string} declared declared test_strategy (may be empty).
 * @param {string[]} changedFiles repo-relative changed paths.
 * @param {{env?: object, ts?: string, ticket?: number, root?: string}} [opts]
 * @returns {{disabled: boolean, meetsFloor: boolean|null, lines: string[], record: object|null}}
 */
function advisoryLines(declared, changedFiles, opts = {}) {
  const env = opts.env || process.env;
  if (isDisabled(env)) {
    return { disabled: true, meetsFloor: null, record: null,
      lines: ['test_floor: disabled (TEST_FLOOR_DISABLED=1)'] };
  }
  const result = reconcile(declared, changedFiles, { root: opts.root });
  const record = auditRecord(result, { ts: opts.ts || null, ticket: opts.ticket || null });
  const lines = [
    `test_floor_declared: ${declared || '(none)'}`,
    `test_floor_derived: ${JSON.stringify(result.derived.codeFloors)} | stress_required: ${result.derived.stressRequired}`,
    `test_floor_meets: ${result.meetsFloor ? 'yes' : 'no (advisory)'}`,
  ];
  for (const gap of result.gaps) lines.push(`test_floor_gap: ${gap}`);
  return { disabled: false, meetsFloor: result.meetsFloor, lines, record };
}
...
module.exports = {
  ...
  auditRecord, isDisabled, advisoryLines,
};
