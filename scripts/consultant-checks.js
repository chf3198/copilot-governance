...
const recurrenceGuard = require('./closeout-recurrence-guard');
...
const checks = [
  ...
  // #3025 AC2: reject a closeout that classifies an already-recurring pattern_id
  // as memory-note-only (recurrence proves a memory note is insufficient). Advisory-first
  // per cross-model consensus Q1 — surfaces as a non-blocking finding unless
  // CLOSEOUT_RECURRENCE_BLOCK=1, in which case it FAILs the closeout.
  [id('gov', 8), 'governance', () => {
    ...
    const result = recurrenceGuard.evaluateCloseout(body);
    const pass = result.status !== 'FAIL';
    const finding = result.status === 'PASS' ? 'no-recurrence-misclassification'
      : (result.status === 'FAIL' ? 'memory-note-only-recurrence-blocked' : 'memory-note-only-recurrence-advisory');
    return ok(id('gov', 8), 'governance', pass, result.message, finding,
      'reclassify the recurring flaw decision from memory-note-only to file-ticket');
  }],
